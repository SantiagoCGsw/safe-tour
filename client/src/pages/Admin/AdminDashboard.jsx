...
function AdminDashboard() {
  ...
  const handleCambiarRol = async (id, rolActual) => {
    const rolesOrden = ['turista', 'gestor', 'admin']
    const siguiente = rolesOrden[(rolesOrden.indexOf(rolActual) + 1) % rolesOrden.length]

    const confirmar = window.confirm(`Cambiar rol a "${siguiente}"?`)
    if (!confirmar) return
    ...
    try {
      ...
      mostrarMensaje('error', 'No se pudo cambiar el rol')
    } finally {
  ...
  return (
    <div style={s.root}>
      ...
      <nav style={s.navbar}>
        ...
        <span style={s.navMarca}>Panel Admin</span>
      ...
      <div style={s.contenido}>
        ...
        <div style={s.tabs}>
          <button
            style={{ ...s.tab, ...(tab === 'stats' ? s.tabActivo : {}) }}
            onClick={() => setTab('stats')}
          >
            Estadísticas
          </button>
          <button
            style={{ ...s.tab, ...(tab === 'usuarios' ? s.tabActivo : {}) }}
        ...
        {tab === 'usuarios' && (
          <div style={s.lista}>
            {usuarios.length === 0 && (
              <p style={s.vacio}>No hay usuarios</p>
            )}
            {usuarios.map(u => (
              <div key={u._id} style={s.item}>
                ...
                {u._id !== usuario?._id && (
                  <button
                    style={{
                      ...s.botonRol,
                      opacity: cambiandoRol === u._id ? 0.6 : 1,
                    }}
                    onClick={() => handleCambiarRol(u._id, u.rol)}
                    disabled={cambiandoRol === u._id}
                    title={`Cambiar a ${rolSiguiente[u.rol]}`}
                  >
                    {cambiandoRol === u._id ? '...' : `→ ${rolSiguiente[u.rol]}`}
                  </button>
                )}
...
const s = {
  ...
  cardsRow: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px' },
