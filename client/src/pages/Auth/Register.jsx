...
function PasswordStrength({ password }) {
  ...
  const colors = ['#ff4444','#ff8800','#f0c800','#00cc88']
  const labels = ['Muy débil','Débil','Buena','Fuerte']
  if (!password) return null
  return (
    <div style={{ marginTop:'6px' }}>
      <div style={{ display:'flex', gap:'4px', marginBottom:'4px' }}>
        {[0,1,2,3].map((i) => <div key={i} style={{ flex:1, height:'3px', borderRadius:'2px', background: i<score ? colors[score-1] : 'rgba(255,255,255,0.1)', transition:'background 0.3s' }}/>)}
      </div>
      <span style={{ fontSize:'0.75rem', color: score>0 ? colors[score-1] : 'transparent' }}>{score>0 ? labels[score-1] : ''}</span>
    </div>
  )
}
...
function Register() {
  ...
  return (
    <div style={s.root}>
      <div style={s.bg}/><div style={s.nebula1}/><div style={s.nebula2}/>
      <StarCanvas/>
      <div style={s.card}>
        ...
        <p style={s.subtitulo}>Crea tu cuenta y explora Bogotá</p>
        <form onSubmit={handleSubmit} style={s.form}>
          ...
          <button style={s.boton} type="submit" disabled={cargando}>
            {cargando ? <span style={s.spinner}/> : 'Crear cuenta'}
          </button>
        </form>
        <p style={s.linkTxt}>¿Ya tienes cuenta? <Link to="/login" style={s.link}>Inicia sesión</Link></p>
      </div>
      <style>{`
...
const s = {
  ...
  boton: { marginTop:'0.4rem', padding:'0.9rem', background:'linear-gradient(135deg,#1565c0,#0d47a1)', color:'#fff', border:'none', borderRadius:'10px', fontSize:'1rem', fontWeight:'600', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', minHeight:'48px' },
