...
const crearIcono = (tipo) => {
  const info =
    iconosPorTipo[tipo?.toLowerCase()] ||
...
function MapPage() {
  ...
  const [reportesLocales, setReportesLocales] =
    useState([])

  const [filtro, setFiltro] =
    useState('todos')

  const [creandoReporte, setCreandoReporte] =
    useState(false)

  const [mostrarFormulario, setMostrarFormulario] =
    useState(false)

  const [posicionNueva, setPosicionNueva] =
    useState(null)

  const [miUbicacion, setMiUbicacion] =
    useState(null)

  const [nuevoReporte, setNuevoReporte] =
    useState({
      titulo: '',
      descripcion: '',
      tipo: 'accidente',
          ...
          {miUbicacion && (
            <Marker
              ...
              icon={L.divIcon({
                className: '',
                html:
                  '<div style="width:20px;height:20px;border-radius:50%;background:#00e5ff;border:3px solid white;box-shadow:0 0 20px #00e5ff;"></div>',
                iconSize: [20, 20],
                iconAnchor: [10, 10],
              })}
            >
